...
SCRAPFLY = ScrapflyClient(key="scp-test-b88386f237944f5ca850fc5e6c61c33e")
...
async def retrieve_comment_params(post_url: str) -> Dict:
    ...
    response = await SCRAPFLY.async_scrape(ScrapeConfig(post_url, **BASE_CONFIG, render_js=True, rendering_wait=5000, wait_for_selector="//div[@id='main-content-video_detail']"))
...
async def scrape_comments(post_url: str, comments_count: int = 20, max_comments: int = None) -> List[Dict]:
    ...
    first_page = await SCRAPFLY.async_scrape(ScrapeConfig(form_api_url(cursor=0), **BASE_CONFIG, headers={"content-type": "application/json"}))
    ...
    _other_pages = [ScrapeConfig(form_api_url(cursor=cursor), **BASE_CONFIG, headers={"content-type": "application/json"}) for cursor in range(comments_count, total_comments + comments_count, comments_count)]
...
def parse_search(response: ScrapeApiResponse) -> List[Dict]:
    ...
    log.debug(f"content: {content}, len {len(content)}")
...
async def obtain_session(url: str) -> str:
    ...
    await SCRAPFLY.async_scrape(ScrapeConfig(url, asp=True, country="US", render_js=True, session=session_id))
    return session_id
...
async def scrape_search(keyword: str, max_search: int, search_count: int = 12) -> List[Dict]:
    ...
    first_page = await SCRAPFLY.async_scrape(ScrapeConfig(form_api_url(cursor=0), asp=True, country="US", session=session_id))
    search_data = parse_search(first_page)

    # scrape the remaining comments concurrently
    log.info(f"scraping search pagination, remaining {max_search // search_count} more pages")
    _other_pages = [ScrapeConfig(form_api_url(cursor=cursor), asp=True, country="US", session=session_id) for cursor in range(search_count, max_search + search_count, search_count)]
    async for response in SCRAPFLY.concurrent_scrape(_other_pages):
...
async def run():
    search_data = await scrape_search(keyword="whales", max_search=10)
